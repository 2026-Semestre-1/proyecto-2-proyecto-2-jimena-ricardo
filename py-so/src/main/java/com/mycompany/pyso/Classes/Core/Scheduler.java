...
public class Scheduler {
    ...
    public boolean admitToRAM(Process process) {
        if (process.getBcp().getState() != ProcessState.NEW) {
            return false;
        }
    ...
    public boolean allTerminated() {
        return jobQueue.getAll().stream().allMatch(p -> p.getBcp().getState() == ProcessState.TERMINATED);
    }
    ...
    private String extractName(String path) {
        return path.substring(path.lastIndexOf(java.io.File.separator) + 1).replace(".asm", "");
    }
    ...
    public static int getKERNEL_SIZE() {
        return KERNEL_SIZE;
    }
